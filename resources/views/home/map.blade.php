...
let [candi, benteng, masjid, astana, taman, kampung, musium, situ, alun, monument, gua, gunung] = [
    "{{ asset('map-icon/candi.png') }}", "{{ asset('map-icon/benteng.png') }}", "{{ asset('map-icon/masjid.png') }}",
    "{{ asset('map-icon/astana.png') }}", "{{ asset('map-icon/taman.png') }}", "{{ asset('map-icon/kampung.png') }}",
    "{{ asset('map-icon/gerbang.png') }}", "{{ asset('map-icon/situ.png') }}", "{{ asset('map-icon/alun.png') }}",
    "{{ asset('map-icon/monumen.png') }}", "{{ asset('map-icon/gua.png') }}", "{{ asset('map-icon/gunung.png') }}"
];
...
//custom icons
let [Candi, Benteng, Masjid, Astana, Taman, Kampung, Musium, Situ, Alun, Monument, Gua, Gunung] = [new MapIcon({
    iconUrl: candi
}), new MapIcon({
    iconUrl: benteng
}), new MapIcon({
    ...
    $("#propinsi").change(function() {
        fetch('https://x.rajaapi.com/poe')
            ...
            .then(r => {
                ...
                $.ajax({
                    url: 'https://x.rajaapi.com/MeP7c5ne' + r.token +
                        '/m/wilayah/kabupaten',
                    ...
                    success: function(json) {
                        ...
                        if (json.code == 200) {
                            for (i = 0; i < Object.keys(json.data).length; i++) {
                                $('#kabupaten').append($('<option>').text(json.data[i]
                                        .name)
                                    .attr('value', json.data[i].id));
                            }
                            $('#kecamatan').html($('<option>').text(
                                    '-- Pilih Kecamatan --')
                                .attr('value', '-- Pilih Kecamatan --'));
                            $('#kelurahan').html($('<option>').text(
                                    '-- Pilih Kelurahan --')
            ...
            .then(r => {
                ...
                $.ajax({
                    ...
                    success: function(json) {
                        ...
                        if (json.code == 200) {
                            for (i = 0; i < Object.keys(json.data).length; i++) {
                                $('#kecamatan').append($('<option>').text(json.data[i]
                                        .name)
                                    .attr('value', json.data[i].id));
                            }
                            $('#kelurahan').html($('<option>').text(
